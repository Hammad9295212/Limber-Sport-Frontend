...
@Component({
  selector: 'app-sport-connections',
  standalone: true,
  imports: [
    Header2Component,
    NgForOf,
  ...
  templateUrl: './sport-connections.component.html',
  styleUrl: './sport-connections.component.css'
})
export class SportConnectionsComponent implements OnInit {
  ...
  async onPlayerDisconnect(id : any) {
    try {
      ...
      const queryParams = {
        other_user_id : id,
        blocked_at: this.formatDate()
      };
      console.log(queryParams);
      this.mainService.onGetUsingToken('api/v1/user-connection-block', queryParams, token).subscribe((data) => {
        console.log(data);
        if (data?.status === 200) {
