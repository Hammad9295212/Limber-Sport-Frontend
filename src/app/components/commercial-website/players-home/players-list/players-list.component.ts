...
@Component({
  ...
  standalone: true,
  imports: [SharedModule, CommonModule, PlayercardComponent, NgbToast, Header2Component, FeaturedComponent],
...
export class PlayersListComponent implements OnInit, AfterViewInit {
  ...
  abilityData: any
  genderData: any
  isPlayersList: boolean = true;
  isPlayersCard: boolean = false;
  ...
  async onGetFilterData(queryParams: any, shouldScroll = true, cancelBarHide = false) {
    ...
    this.mainpageService.onGet(player?.endPoint, obj).subscribe((data) => {
      ...
      this.playerListData = mainData;
      this.playerListDataBackup = mainData;
      console.log(this.playerListDataBackup);
  ...
  handlePlayersView(a: any) {
    if (a === 'list') {
      this.isPlayersCard = false;
  ...
  onFilter(ev: any) {
    ...
    if (this.searchTerm?.length > 0) {
      this.playerListData = this.playerListDataBackup.filter((item: any) => {
  ...
  onClearFilter() {
    ...
    (document.getElementById('lookingFor0') as HTMLInputElement)?.click();
    (document.getElementById('sportsSelection') as HTMLInputElement)?.click();
    (document.getElementById('ability_idSelection') as HTMLInputElement)?.click();
    (document.getElementById('ageRangeSelection') as HTMLInputElement)?.click();
    (document.getElementById('tagsfields') as HTMLInputElement)?.click();
    (document.getElementById('looking_for-clear') as HTMLInputElement)?.click();
    this.extra.isStoppedFetching = false;
    this.queryParams.role_id = 1;
    this.onGetFilterData({role_id: 1});
  }
  ...
  onSort(ev: any) {
    ...
    if (searchTerm?.length > 0) {
      if (searchTerm === "location") {
        // Sort the playerListDataBackup array by the location inside the player array
        this.playerListData = this.playerListDataBackup.sort((a: { player: { location: string; }[]; }, b: {
  ...
  onGetPlayerDetail(token: any, user_id: any) {
    this.mainpageService.onGetUsingToken(`api/v1/get-player-details`, {user_id}, token).subscribe((data) => {
